"""
Movie registration chatbot:
Chatbot for movie registration at Zaisan Cinema:
By Tuul Bayasgalan
"""
...
movies = ('Batman', 'Joker', 'Spider-man')
...
def pick_movie(movies):
    ...
    while movie_choice == None:
        movie_choice = input ('Enter Film Option Number: ')
        try:
...
def pick_time(periods):
    ...
    print()
    print("Movie time Schedule for today")
    print ('-------------')
...
def pick_seat(movie_index, period_index, ordered_seats):
    ...
    while seat_choice not in range(1, 16):
        try:
            seat_choice = int(input("What seat would you like to book? "))
        except:
...
#Beginning of bot
print("Welcome to Zaisan Movie Registration")
...
while rebook == 'yes':
    ...
    while rebook not in ('yes', 'no'):
        rebook = (input("Would you like to restart your order? ")).lower()
        if rebook == 'yes':
            print('Rebooking...')
        elif rebook == 'no':
